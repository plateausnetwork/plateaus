...
rem Change parameter token denominations to axrz
cat %GENESIS% | jq ".app_state[\"staking\"][\"params\"][\"bond_denom\"]=\"axrz\""   >   %TMPGENESIS% && move %TMPGENESIS% %GENESIS%
cat %GENESIS% | jq ".app_state[\"crisis\"][\"constant_fee\"][\"denom\"]=\"axrz\"" > %TMPGENESIS% && move %TMPGENESIS% %GENESIS%
cat %GENESIS% | jq ".app_state[\"gov\"][\"deposit_params\"][\"min_deposit\"][0][\"denom\"]=\"axrz\"" > %TMPGENESIS% && move %TMPGENESIS% %GENESIS%
cat %GENESIS% | jq ".app_state[\"mint\"][\"params\"][\"mint_denom\"]=\"axrz\"" > %TMPGENESIS% && move %TMPGENESIS% %GENESIS%
...
evmosd add-genesis-account %KEY% 100000000000000000000000000axrz --keyring-backend %KEYRING%

rem Sign genesis transaction
evmosd gentx %KEY% 1000000000000000000000axrz --keyring-backend %KEYRING% --chain-id %CHAINID%
...
evmosd start --pruning=nothing %TRACE% --log_level %LOGLEVEL% --minimum-gas-prices=0.0001axrz