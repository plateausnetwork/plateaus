...
var (
	now  = time.Now().UTC()
	acc1 = sdk.MustAccAddressFromBech32("xrz1hmvjlsellna3khhfn2n5d2mmxucr9jmyqul9s0")
	acc2 = sdk.MustAccAddressFromBech32("xrz1777uf283z705lmeztqxpme7w7e6g4qjlwdy2ch")
)
...
func (suite *GenesisTestSuite) TestClaimInitGenesis() {
	testCases := []struct {
		...
		},
		{
			...
			func() {
				coins := sdk.NewCoins(sdk.NewCoin("axrz", sdk.NewInt(2_800)))
		...
		},
		{
			...
			func() {
				coins := sdk.NewCoins(sdk.NewCoin("axrz", sdk.NewInt(400)))
				err := testutil.FundModuleAccount(suite.app.BankKeeper, suite.ctx, types.ModuleName, coins)
				suite.Require().NoError(err)
			},
			false,
