...
LEDGER_ENABLED ?= true
BINDIR ?= $(GOPATH)/bin
PLATEAUS_BINARY = plateausd
PLATEAUS_DIR = plateaus
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=plateaus \
          -X github.com/cosmos/cosmos-sdk/version.AppName=$(PLATEAUS_BINARY) \
...
godocs:
	@echo "--> Wait a few seconds and visit http://localhost:6060/pkg/github.com/rhizomplatform/plateaus/types"
	godoc -http=:6060
...
test-sim-custom-genesis-fast:
	...
	@echo "By default, ${HOME}/.$(PLATEAUS_DIR)/config/genesis.json will be used."
	@go test -mod=readonly $(SIMAPP) -run TestFullAppSimulation -Genesis=${HOME}/.$(PLATEAUS_DIR)/config/genesis.json \
...
test-sim-custom-genesis-multi-seed: runsim
	...
	@echo "By default, ${HOME}/.$(PLATEAUS_DIR)/config/genesis.json will be used."
	@$(BINDIR)/runsim -Genesis=${HOME}/.$(PLATEAUS_DIR)/config/genesis.json -SimAppPkg=$(SIMAPP) -ExitOnFail 400 5 TestFullAppSimulation
...
format:
	...
	find . -name '*.go' -type f -not -path "./vendor*" -not -path "*.git*" -not -path "./client/docs/statik/statik.go" -not -name '*.pb.go' | xargs goimports -w -local github.com/rhizomplatform/plateaus
.PHONY: format
...
.PHONY: build-docker-local-plateaus localnet-start localnet-stop
