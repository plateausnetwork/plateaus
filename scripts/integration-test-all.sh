...
## default port prefixes for plateausd
NODE_P2P_PORT="2660"
NODE_PORT="2663"
NODE_RPC_PORT="2666"
...
init_func() {
    "$PWD"/build/plateausd keys add $KEY"$i" --keyring-backend test --home "$DATA_DIR$i" --no-backup --algo "eth_secp256k1"
    "$PWD"/build/plateausd init $MONIKER --chain-id $CHAINID --home "$DATA_DIR$i"
    "$PWD"/build/plateausd add-genesis-account \
    "$("$PWD"/build/plateausd keys show "$KEY$i" --keyring-backend test -a --home "$DATA_DIR$i")" 1000000000000000000axrz,1000000000000000000stake \
    --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/plateausd gentx "$KEY$i" 1000000000000000000stake --chain-id $CHAINID --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/plateausd collect-gentxs --home "$DATA_DIR$i"
    "$PWD"/build/plateausd validate-genesis --home "$DATA_DIR$i"
...
start_func() {
    ...
    "$PWD"/build/plateausd start --pruning=nothing --rpc.unsafe \
    ...
    PLATEAUS_PID=$!
    echo "started plateaus node, pid=$PLATEAUS_PID"
    # add PID to array
    arr+=("$PLATEAUS_PID")
...
if [[ -z $TEST || $TEST == "rpc" ||  $TEST == "pending" ]]; then
    ...
    for i in $(seq 1 "$TEST_QTD"); do
        ...
        echo "going to test plateaus node $HOST_RPC ..."
...
stop_func() {
    PLATEAUS_PID=$i
    echo "shutting down node, pid=$PLATEAUS_PID ..."

    # Shutdown plateaus node
    kill -9 "$PLATEAUS_PID"
    wait "$PLATEAUS_PID"
